...
def flyer_get_packet(media, target_board_id):
    ...
    global total_receive_bytes
    global receive_packet_done
        ...
        if (state == GET_PACKET_STATE_RECEIVE_DATA):
            ...
            if (board_id != FLYER_BOARD_ID or packet_type != PACKET_TYPE_DATA):
                print("rcving error! packet id: %d, type %d, expect id: %d, type %s "
                        %(board_id, packet_type,  FLYER_BOARD_ID, pt_str[PACKET_TYPE_DATA]))
                state = GET_PACKET_STATE_QUIT
                continue
            ...
            total_receive_bytes += len(packet)
            _packet_seq += 1
            state = GET_PACKET_STATE_SEND_ACK
...
def flyer_receive_packet_from_station(media, id):
    ...
    while time.time() < cur_time + 20.0:
        ...
        if (receive_packet_done):
            ...
            print("receive %d bytes in %.3f" %(total_receive_bytes, time_interval))
            print("speed: %.2fKB/s" %((total_receive_bytes/1000)/time_interval))
    ...
    while True:
        ...
        if (state == SEND_PACKET_STATE_WAIT_ACK):
            ...
            if (send_packet_cnt == packets_cnt):
                print("station received packets %d" %(send_packet_cnt))
                return True
        ...
        if (state == SEND_PACKET_STATE_SEND):
            ...
            if (send_packet_cnt != packets_cnt - 1):
                crc_str = CRC_LEN * '\0'
                packet =  packet +  sendfile.read(PACKET_DATA_LEN) + crc_str
            else:
...
while True:
    cmd = raw_input("input s to start request packet from station   ")
    if (cmd == 's'):
        flyer_receive_packet_from_station(media, STATION_BOARD_ID)
